...
// Keep lighting behavior synchronized with opaque.frag.
...
layout(location = 5) flat in vec4 SurfaceNormalAndRule;
layout(location = 6) flat in vec4 SurfaceTilingOffset;
...
bool OnSurfaceFace(vec3 n) {
    float rule = SurfaceNormalAndRule.w;
    if (rule > 1.5) {
        return abs(dot(n, SurfaceNormalAndRule.xyz)) < SURFACE_AROUND_MATCH;
    }
    if (rule > 0.5) {
        return true;
    }
    return dot(n, SurfaceNormalAndRule.xyz) > SURFACE_FACE_MATCH;
}
...
void main() {
    ...
    if (partFragment.HasSurfaceTexture.x > 0.5 && OnSurfaceFace(n)) {
        vec2 surfaceUV = (FragmentUV * SurfaceTilingOffset.xy) + SurfaceTilingOffset.zw;
