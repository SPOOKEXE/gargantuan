...
layout(set = 3, binding = 1) uniform Params {
    ...
    vec4 NightVision;
    vec4 Contrast;
    // Automatic gain lifts shadows more than highlights.
    vec4 Gain;
} params;
...
void main() {
    ...
    if (night > 0.0) {
        // IR mode is monochrome with high gain.
        float gained = clamp(pow(luma, 0.8) * 1.15, 0.0, 1.0);

        // Four-tap bloom mimics a wide-open sensor.
        ...
        // Gamma gain lifts shadows without clipping highlights.
    ...
    // Time-shifted alternating rows mimic interlaced fields.
    ...
    // Noise scales with darkness and IR gain.
