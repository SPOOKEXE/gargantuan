...
namespace gargantuan {
	...
	namespace {
		// Round byte blends to nearest to prevent cumulative downward drift.
	...
	void EditableImage::CombinePixel(
		...
		// Coverage and source alpha are distinct; Overwrite must preserve source alpha.
		...
		// Straight-alpha compositing must weight the destination by its alpha.
		int inverse = 255 - alpha;
	...
	namespace {
		// Approximate circle coverage on a 4x4 sample grid.
	...
	void EditableImage::DrawCircle(
		...
		// Integer coordinates name pixels; one-pixel lines must cover both endpoints.
	...
	void EditableImage::DrawLine(
		...
		// Cover once to prevent overlap from darkening translucent lines.
	...
	int EditableImage::LSave(lua_State *L, Instance *instance) {
		...
		// An explicit format overrides the path extension.
	...
	bool EditableImage::Load(std::string path) {
		...
		if (!decoded.WasDecoded) {
			LoadError = decoded.ErrorMessage;
			return false;
		}

		LoadError.clear();
		SetPixels(decoded.Width, decoded.Height, decoded.RGBA8Pixels.data());
		return true;
	}
	...
	int EditableImage::LReadPixelsBuffer(lua_State *L, Instance *instance) {
		...
		// Preserve requested dimensions; out-of-image pixels remain zero.
		size_t pixelByteCount = (size_t)requestedWidth * requestedHeight * CHANNELS;
		void *data = lua_newbuffer(L, pixelByteCount);
		std::memset(data, 0, pixelByteCount);
