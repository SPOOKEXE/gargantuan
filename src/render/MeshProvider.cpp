...
namespace gargantuan::MeshProvider {
	uint64_t GpuMeshCacheGeneration = 1;
	namespace {
		std::unordered_map<std::string, Mesh> PendingCpuMeshes = {
	...
	void DestroyAllGpuMeshes(SDL_GPUDevice *gpu) {
		...
		GpuMeshes.clear();
		GpuMeshCacheGeneration++;
	}

	void UploadToGpu(SDL_GPUDevice *gpu) {
		if (PendingCpuMeshes.empty()) {
			return;
		}
		...
		for (auto &[meshId, unloadedMesh] : PendingCpuMeshes) {
