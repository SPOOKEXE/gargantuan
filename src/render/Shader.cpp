...
namespace gargantuan {
	...
	void ShaderProgram::Destroy(SDL_GPUDevice *gpu) {
		if (VertexShader) {
	...
	SDL_GPUShader *
	FileShader::CreateShaderFromFile(SDL_GPUDevice *gpu, std::filesystem::path filepath, SDL_GPUShaderCreateInfo info) {
		size_t bytecodeSize;
		void *bytecode = SDL_LoadFile(filepath.string().c_str(), &bytecodeSize);
		if (bytecode == nullptr) {
		...
		info.code = static_cast<const Uint8 *>(bytecode);
	...
	void FileShader::Init(SDL_GPUDevice *gpu) {
		...
		if (!VertexShader) {
			std::string filepath = VertexFilepathStem.string() + extension;
		...
		if (!FragmentShader) {
			std::string filepath = FragmentFilepathStem.string() + extension;
